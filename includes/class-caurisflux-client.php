...
final class CaurisFlux_Client {
	...
	private function request( string $method, string $path, ?array $body = null, array $extra_headers = array() ): array {
		if ( '' === $this->api_key || false === strpos( $this->api_key, ':' ) ) {
			return array(
				'success' => false,
				'error'   => __( 'CaurisFlux API key is invalid or not configured.', 'caurisflux-for-woocommerce' ),
				'status'  => 0,
			);
		}
		...
		if ( null !== $body && in_array( $method, array( 'POST', 'PUT', 'PATCH' ), true ) ) {
			...
			if ( false === $encoded ) {
				return array(
					'success' => false,
					'error'   => __( 'Unable to serialize the request to JSON.', 'caurisflux-for-woocommerce' ),
					'status'  => 0,
				);
			}
			$args['body'] = $encoded;
		...
		return array(
			'success' => false,
			'error'   => '' !== $last_error ? $last_error : __( 'CaurisFlux API error', 'caurisflux-for-woocommerce' ),
			'data'    => $last_data,
			'status'  => $last_status,
		);
	}
